...
function NovoClienteModal({
  ...
  // Usa os nomes reais dos templates de serviço (Configurações > Templates
  // Operacionais) — antes essa lista era um texto solto, sem ligação
  // nenhuma com os templates de verdade, então o checklist nunca batia
  // com o serviço escolhido aqui (ficava sempre vazio, 0/0).
            ...
            <div className="flex flex-wrap gap-1.5">
              {serviceTemplates.map((t) => (
                <button
                  key={t.id}
                  type="button"
                  onClick={() => toggleServico(t.name)}
                  className={cn(
                    "rounded-md border px-2 py-1 text-[11px] font-medium transition-colors",
                    services.includes(t.name)
